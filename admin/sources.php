...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        case 'add':
            ...
            $parserUrl = trim($_POST['parser_url']);
            if (empty($name) || empty($apiUrl)) {
                $message = '名称和API地址不能为空';
                $messageType = 'error';
            } else {
        ...
        case 'toggle':
            ...
            $db->update('play_sources', array('status' => $newStatus), 'id = ?', array($id));
            $message = $newStatus ? '播放源已启用' : '播放源已禁用';
            $messageType = 'success';
            break;
...
if (isset($_GET['edit'])) {
    ...
    $editSource = $db->fetchOne("SELECT * FROM play_sources WHERE id = ?", array($editId));
}

if (isset($_GET['test'])) {
    $testId = intval($_GET['test']);
    $testSource = $db->fetchOne("SELECT * FROM play_sources WHERE id = ?", array($testId));
    if ($testSource) {
        $apiUrl = $testSource['api_url'];
        $context = stream_context_create(['http' => ['timeout' => 5]]);
        $response = @file_get_contents($apiUrl, false, $context);
        if ($response !== false) {
            $message = '播放源连接成功！API响应正常。';
            $messageType = 'success';
        } else {
            $message = '播放源连接失败！无法访问API。';
            $messageType = 'error';
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .status-badge.default { background: rgba(255,183,3,0.15); color: #ffb703; }
        .source-url { color: var(--muted); font-size: 0.8rem; word-break: break-all; max-width: 280px; display: inline-block; }
    ...
    <div class="admin-layout">
        <aside class="admin-sidebar">
            ...
            <nav class="admin-sidebar-nav">
                ...
                <div class="admin-nav-section">
                    ...
                    <a href="sources.php" class="admin-nav-item active"><span class="nav-icon">📺</span>播放源管理</a>
        ...
        <main class="admin-main">
            ...
            <div class="form-panel">
                <h2><?php echo $editSource ? '✏️ 编辑播放源' : '➕ 添加新播放源'; ?></h2>
                <form method="post">
                    <?php if ($editSource): ?>
                    <input type="hidden" name="action" value="edit">
                    <input type="hidden" name="id" value="<?php echo $editSource['id']; ?>">
                    <?php else: ?>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" placeholder="例如: 默认播放源" value="<?php echo $editSource ? clean($editSource['name']) : ''; ?>" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">解析器URL</label>
                            <input type="text" name="parser_url" class="form-control" placeholder="解析播放器地址" value="<?php echo $editSource ? clean($editSource['parser_url']) : ''; ?>">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">API地址 *</label>
                        <input type="text" name="api_url" class="form-control" placeholder="https://api.example.com/apijson.php" value="<?php echo $editSource ? clean($editSource['api_url']) : ''; ?>" required>
                    </div>
                    <div style="display: flex; gap: 12px;">
                        <button type="submit" class="btn-primary"><?php echo $editSource ? '保存修改' : '添加播放源'; ?></button>
                        <?php if ($editSource): ?>
                        <a href="sources.php" class="btn-secondary" style="display: inline-flex; align-items: center; justify-content: center; text-decoration: none;">取消编辑</a>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
            ...
            <div class="panel">
                <table class="admin-table">
                    ...
                    <tbody>
                        ...
                        <tr><td colspan="7" class="empty-state">暂无播放源</td></tr>
                        <?php else: ?>
                        <?php foreach ($sources as $source): ?>
                        <tr>
                            <td style="color: var(--muted);">#<?php echo $source['id']; ?></td>
                            <td style="font-weight: 600;"><?php echo clean($source['name']); ?></td>
                            <td><span class="source-url"><?php echo clean($source['api_url']); ?></span></td>
                            <td style="color: var(--muted); font-size: 0.8rem;"><?php echo $source['parser_url'] ? clean($source['parser_url']) : '-'; ?></td>
                            <td>
                                <?php if ($source['is_default']): ?>
                                <span class="status-badge default">⭐ 默认</span>
                                <?php else: ?>
                                <form method="post" style="display:inline;">
                                    <input type="hidden" name="action" value="set_default">
                                    <input type="hidden" name="id" value="<?php echo $source['id']; ?>">
                                    <button type="submit" class="action-btn warning">设为默认</button>
                                </form>
                                <?php endif; ?>
                            </td>
                            <td>
                                ...
                                <?php else: ?>
                                <span class="status-badge inactive">⏸️ 禁用</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <div class="table-actions">
                                    <a href="?test=<?php echo $source['id']; ?>" class="action-btn primary">测试</a>
                                    <a href="?edit=<?php echo $source['id']; ?>" class="action-btn success">编辑</a>
                                    <form method="post" style="display:inline;">
                                        <input type="hidden" name="action" value="toggle">
                                        <input type="hidden" name="id" value="<?php echo $source['id']; ?>">
                                        <button type="submit" class="action-btn warning"><?php echo $source['status'] ? '禁用' : '启用'; ?></button>
                                    </form>
                                    <form method="post" style="display:inline;" onsubmit="return confirm('确定删除此播放源？');">
